...
- MapLibre GL (OpenFreeMap `liberty` style, no API key needed) renders `output/geojson/aqi-latest.geojson`, circles colored by the standard EPA AQI breakpoints (0/51/101/151/201/301). The `aqi` source/layer are set up as soon as the map's own `load` event fires (or immediately if `map.loaded()` is already true) — this is intentionally decoupled from the live-data fetch in `init()`; registering `map.on('load', ...)` only after an `await` previously raced the map's own load (tiles cached ⇒ `load` fires first ⇒ listener attaches too late ⇒ source/layer silently never added, no markers, no error).
...
- Each chart draws a smooth min/max "ribbon" — one point per IST calendar day (`computeDailyEnvelope`), rendered as two real Chart.js line datasets (`fill: '+1'` between them) — so the band varies day to day rather than being a single flat range over the whole window. A dashed "current live reading" line is drawn on top via a hand-rolled Chart.js plugin (`minMaxBandPlugin`) — no annotation-plugin dependency was added, consistent with the no-CSV-library approach above.
- Hovering or clicking a chart shows a synced crosshair (`crosshairPlugin`) at the same timestamp across every field's chart for that device, and swaps the reading-tiles grid to that timestamp's values (`renderReadingsGrid`/`applyInspect`) so all params at one moment can be compared directly. Clicking toggles a pin: first click freezes the inspection until a second click (anywhere on a chart) or the "back to live" control releases it — pinning is the primary interaction on touch devices, where there's no hover. Chart.js's native tooltip is disabled (`tooltip: { enabled: false }`); `crosshairPlugin` draws its own value label next to the highlighted dot instead, so the label can never disagree with the highlighted point — with closely spaced readings, Chart.js's own nearest-point search (run independently per chart, and easily confused by the sparser min/max ribbon datasets sharing the same chart) could otherwise highlight a different point than the one `findNearestRow`/`currentDeviceRows` already picked.
